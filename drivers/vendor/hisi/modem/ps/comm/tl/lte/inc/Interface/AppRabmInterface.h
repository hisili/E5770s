...
extern VOS_UINT32 APPITF_SaveDataToRabmBuf( LUP_MEM_ST *pstMem );


extern VOS_VOID  APPITF_FreeAllofRabmBuf( VOS_VOID );


extern VOS_VOID APPITF_SendDataofRabmBuf( VOS_VOID );

/*****************************************************************************
 Function Name   : Nas_DataReq
 Description     : NAS提供给APP的函数，由APP调用
 Input           : VOS_UINT32 ulDataLen, LUP_MEM_ST *pstSdu
 Output          : None
 Return          : APP_ERABM_TRANSFER_RESULT_SUCC  调用成功，调用者无需关心资源的释放
                   APP_ERABM_TRANSFER_RESULT_FAIL  调用失败，调用者需要释放相关资源
